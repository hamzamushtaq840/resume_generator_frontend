...
function Profile() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/getUser/" + user.id, { withCredentials: true })
      .then((res) => {
        ...
        setInput(res.data.data[0].location)
        setDescription(res.data.data[0].description)
        setImageURL(res.data.data[0].img)
  ...
  const handleJob = (e) => {
    ...
    if (
      users.name === name
      && (users.description === null && description === '' || users.description === description)
      ...
      if (imageURL === '' && users.img === null) {
        data.img = null
      }

      axios.post("http://localhost:5000/api/updateCompanyProfile", data, { withCredentials: true })
        .then((res) => {
          console.log(res);
          if (res.status === 200) {
        ...
        <div className={styles.formmm}>
          ...
          >
            <TextField
              ...
              label="Company Name"
              value={name}
              helperText="This name will be shown for jobs"
              variant="standard"
              name="title"
